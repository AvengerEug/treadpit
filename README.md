...
#### 2.11.4 jvm young gc日志分析（cms收集器）
...
[Times: user=0.02 sys=0.00, real=0.01 secs]：GC操作中的cpu时间

```

#### 2.11.5 jvm full gc日志分析（cms收集器）

```text
2018-04-29T10:16:39.810-0200: [Full GC (CMS Final Remark) [CMS: 10432K->16896K(175104K), 0.0183673 secs] 21184K->16896K(251392K), [Metaspace: 6745K->6745K(1056768K)], 0.0184773 secs] [Times: user=0.05 sys=0.00, real=0.02 secs]

Full GC：表示这是一次老年代的完全垃圾回收。
CMS Final Remark：触发Full GC的原因，这里是CMS收集器的最终标记阶段。
[CMS: 10432K->16896K(175104K), 0.0183673 secs]：回收前老年代的使用量和回收后的使用量，以及老年代的总大小，以及GC耗时。
21184K->16896K(251392K)：回收前整个堆的使用量和回收后的使用量，以及堆的总大小。
[Metaspace: 6745K->6745K(1056768K)]：回收前和回收后元空间的使用量，以及元空间的总大小。
[Times: user=0.05 sys=0.00, real=0.02 secs]：GC操作中的CPU时间。
...
* CMS(Concurrent Mark Sweep)收集器是jvm的一种以获取最短回收停顿时间为目标的垃圾收集器，主要是用来清理老年代的堆数据。老年的垃圾回收操作一共有两种：major gc（清理老年代） 和 full gc（年轻代和老年代都清理）
* 工作过程：
  * 初始标记(Initial Mark)：先标记一些root对象
  * 并发标记(Concurrent Mark)：遍历堆中的所有对象，并且标记所有可达对象
  * 重新标记(Remark)：重新标记因为程序运行而产生的变动，是并发标记的一种补偿机制
  * 并发清除(Concurrent Sweep)：并发的清除未被标记的对象，释放内存
...
* 缺点：
  ...
  * CMS回退到Serial Old收集器(oncurrent mode failed和promotion failed关键字)：当老年代空间不足无法完成垃圾回收时，会回退到使用Serial Old收集器的fullgc，会停顿较长时间。

#### 2.11.8 jvm gc日志的几个日志报错关键字（cms收集器）
...
* promotion failed（**full gc的相关关键字**）：**是指对象晋升到老年代时失败**。在垃圾收集过程中，当年轻代中的对象达到一定年龄时（默认15，可以配置），他们会被晋升到老年代。如果老年代空间不足以容纳即将晋升的对象，晋升操作会失败。这通常会触发一次fullgc(退化成Serial Old收集器的fullgc，非常耗时)，以清理老年代并尝试为这些对象腾出空间。
* oncurrent mode failed（**full gc的相关关键字**）：是指在使用cms垃圾收集器过程中，由于**老年代空间不足**，导致并发模式无法成功完成。"在cms的并发收集周期中，应用程序的线程会和垃圾收集线程通知运行。如果在这个周期内老年代空间变得满溢，CMS无法继续并发地收集垃圾，因此会退回到Serial Old收集器的fullgc，即会停止所有的应用线程，直到垃圾收集完成。这会导致较长的停顿时间，并可能影响应用程序的性能"
...
* jvm参数
  ...
  -XX:ArrayAllocationWarningSize=15m
  -Xmn64m
...
| 类型  | 优势                                                         | 使用场景                                                     |
| ----- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| NoSQL | 强扩展性：NoSQL数据库通常设计为分布式系统，可以增加更多服务器来实现水平扩展<br>灵活性：NoSQL数据库不强制使用固定的表结构，允许动态添加或更改数据模型。非常适合于数据结构多变的应用程序<br>性能：它们的存储和查询机制简单直接，减少了复杂的关系处理过程<br>高可用性和分区容错：NoSQL数据库经常内置复制和数据分区功能，即使在服务器故障时也能保证服务持续可用性<br>多样化的数据存储能力：支持多种数据模型，如键值对、文档、时序、宽表、文件等 | 电子商户：通过mongoDB存储和处理庞大的用户和商品数据，并通过redis对高吞吐场景进行加速<br>内容管理系统：在线出版和新闻网站需要管理大量的文章、图片和视频内容。NoSQL数据库能够支持大规模的内容存储，并提供快速的数据访问，以应对用户的阅读请求。例如，阿里云的[云数据库MongoDB版](https://www.aliyun.com/product/mongodb)支持多种数据格式的存储，非常适合内容管理系统中多样化的内容需求。 |

* **典型的NoSQL数据库**

| NoSQL数据库    | 说明                                                         | 特点                                                         | 使用案例                                  | 举例           |
| -------------- | ------------------------------------------------------------ | ------------------------------------------------------------ | ----------------------------------------- | -------------- |
| kv数据库       | 以键值对的形势存储数据，值支持多种数据结构：String、Hash、List、Set、SortedSet等 | 超高读写性能支持多种数据结构                                 | 缓存购物车排行榜秒杀防超卖                | redistair      |
| 文档数据库     | 以文档的形势存储数据，文档格式通常为json、bson、xml等        | 结构灵活，每个文档可以有不同字段，增减字段无需停机活修改数据库表schema支持嵌套文档，避免复杂的关联操作支持二级索引，可以使用非主键字段进行查询 | 内容管理游戏应用社交应用用户/角色属性管理 | mongodb        |
| 列式存储数据库 | 按列组织数据                                                 | 压缩率高适合大量数据                                         | 数据分析监控物联网                        | hbase、lindorm |
| 多模数据库     | 支持在一个数据库里存储、查询、分析多种数据模型。例如：宽表、时序、文本、对象、流、空间等 | 性能高压缩率高支持统一查询接口，以及跨多模引擎关联查询支持海量数据实时存储与分析 | 物联网车联网大数据                        | lindorm        |

* NoSQL数据库举例

| 名称     | 类型  | 优势                                                         | 使用场景                                                     |
| -------- | ----- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| Hologres | NoSQL | 实时场景实时写入、更新，写入即可见亚秒级交互式分析无需预计算，支持多维分析开放生态标准的SQL接口，兼容PostgreSQL协议，无额外学习使用成本支持JSON等半结构化数据MaxCompute查询加速与MaxCompute无缝对接，无需导入导出即可查询MaxCompute表的数据计算存储分离架构支持动态升降配。资源越多，查询的并发数量越大 | **智能的数据分析加速器**，它能让你在**超大规模数据集**上进行实时分析，就像在小数据上操作一样快。关键：**超大数据量的快速数据分析。用于解决MySQL在数据量大的情况下导致的性能问题**实际场景：需要存储大数据量并且需要类似mysql一样的查询，则选它 |
| HBase    | NoSQL |                                                              | 超大的表格，里面存放**海量**的数据，而且这个数据还在不断的快速增加。**适用于快速读写大量数据的场景。**关键：快速读写**大量**数据（用于替换redis内存不够的场景）redis有读瓶颈，如果redis不够，则用hbase |
| Redis    | NoSQL |                                                              | 超级快的记忆库，把数据存在**内存**中，提高访问速度。关键：内存，提速。决定了数据量不能太大 |
| MySQL    |       |                                                              | 数据库管家。适合处理关系型数据（比如一个用户可以有多个订单，可以用join查询） |
| MongoDB  | NoSQL |                                                              | 关键：不需要事务无关联查询业务复杂（字段频繁变动）水平扩容快 |

* **与关系型数据库的区别**

| 特性     | nosql                                                        | 关系型数据库                                      |
| -------- | ------------------------------------------------------------ | ------------------------------------------------- |
| 数据模型 | 非结构化数据（键值对、文档、宽表、文件）                     | 表格模型，严格的二维表结构                        |
| 架构     | 分布式，水平扩展（添加节点）                                 | 集中式，垂直扩展（增强机器的性能，eg：cpu和内存） |
| 数据结构 | 非固定                                                       | 固定的数据结构，变更需要修改全量数据结构          |
| 查询语言 | 各自独有                                                     | 标准化的sql                                       |
| 事务     | 部分支持                                                     | 支持ACID                                          |
| 索引     | 支持，但无关系型灵活                                         | 支持，严格的索引结构                              |
| 适用场景 | 大数据处理，在线应用，简单查询，以及对数据灵活性、弹性能力有高要求的场景 | 事务性强，数据关系复杂                            |

#### 2.14.8 **如何选择合适的数据库**

- **如果有很大量的数据要缓存的话，就把redis升级成hbase**
- **如果有很大量的数据查询需求，并且也要事务的话，就把mysql升级成hologres**
- **如果想快速迭代业务，遇到db瓶颈问题想快速解决并支撑业务的话：就用mongoDB**
...
* 资源埋点和规则配置分离，客户端负责埋点（引入sentinel包的应用），控制台负责管理规则（对埋点的限流/降级规则）并推送。

#### 2.17.3 如何接入

* 应用引入sentinel二方包，并结合sentinel的控制台完成对资源的流控配置
* Web应用使用的是WebFilter完成限流操作。

#### 2.17.4 规则汇总

* 限流规则&限流模式
  * 直接限流：根据限流资源自己的调用量进行限流阈值控制
  * 关联限流：A资源的限流是根据关联资源B的调用量达到限流阈值而控制
  * 链路限流：A资源的限流只统计从关联的链路过来的调用量进行限流阈值控制
* 降级规则：
  * 目标：保护服务不被下游服务拖死
  * 方式：不可用的服务快速失败
    * 当前RT超过阈值并且请求数在5以上时触发降级；
    * **降级窗口**内对下游的所有调用都会立即抛出异常；
* 热点规则：
  * 目标：保护服务提供方不被过于频繁的调用压垮
  * 方式：过于频繁的调用直接拒绝
    * 区分接口的参数值，参数不同的值分别限流；
    * 如果参数是数组或collection，会将其遍历展开
* 系统保护规则：
  * 根据全局load、全局rt、全局qps对系统做保护
...
* 格式： `watch [类名] [方法名] "{params, target, returnObj}" [条件表达式] [动作] [选项]`
  * `[类名]` 是要观察的类名。
  * `[方法名]` 是要观察的方法名。
  * `{params, target, returnObj}` 是可选项，可以按需观察方法的参数 (`params`)、调用实例(`target`)、返回值(`returnObj`)或抛出的异常(`throwExp`)。
...
| 名词                    | 作用                                                         | 备注                                                         |
| ----------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| kebe-apiserver          | 对外暴露k8s的api接口，是外界操作k8s资源的唯一入口。提供：认证、授权、访问控制、api注册和发现等机制 |                                                              |
| etcd                    | 是k8s所有集群数据的后台数据库                                |                                                              |
| kebe-scheduler          | 主节点上的主键，该组件监视那些新创建的未指定运行节点的pod，并选择节点让pod在上面运行。所有k8s集群的操作，都必须经过主节点进行调度 |                                                              |
| kebe-controller-manager | 在主节点上的控制器组件。可以用来控制节点、副本、端点、服务上号和令牌控制器 |                                                              |
| kebelet                 | 一个运行在集群中每个节点的组件。负责接收master节点的相关操作指令，主要负责维护容器的生命周期，同时也负责Volume和网络的管理 |                                                              |
| kebe-proxy              | 负责为service提供pod的服务发现和负载均衡                     |                                                              |
| container               | 容器，对应的就是一个docker的容器                             |                                                              |
| pod                     | k8s的最小部署单元。pod里可以启动多个容器，一个pod中的所有容器共享同一网络。 |                                                              |
| volume                  | 声明在pod容器中可访问的文件目录。通过一系列定义，可以让一个pod中的多个容器共享挂载的目录 | 可以挂载成emptyDir和hostPath等多种模式。emptyDir模式，表示一个pod中的多个容器可以共享同一个目录。hostPath模式表示可以将node上的信息挂载到pod中，并且不会随着pod消失而消失 |
| controller              | 节点、副本、端点、服务上号和令牌控制器 都有对应的控制器，同时可以定义一次性任务和定时任务 |                                                              |
| deployment              | 定义一组pod的副本数目、版本等。是多个pod一个抽象             |                                                              |
| service                 | 定义一组pod的访问策略。pod的负载均衡，提供一个或多个pod的稳定访问地址 |                                                              |
| label                   | 标签。可用于对象资源的查询，筛选。service就是通过label来对pod做服务发现 |                                                              |
| namespace               | 一个集群内部的逻辑隔离机制。每个资源都属于一个namespace，同一个namespace的资源名不能重复。 |                                                              |
| configmap               | 理解成java中的一个map，用来存储key、value的信息。每个资源都可以从configmap中取数据（同namespace下） | 可以把config放入到pod的系统环境变量中。也可以把它挂载到pod的volumn中，此时可以支持热更新（configmap的内容变化后，volumn也会感知到，大概有10s的延迟） |
| secret                  | 也可以理解成一个map，但是它通常是用来对敏感数据做加密操作的。防止泄露 |                                                              |
|                         |                                                              |                                                              |
|                         |                                                              |                                                              |
***
...
* top
* 指定某个线程: top | grep 进程名
* 每个进程的%cpu列表示：当前线程占用的进程使用率。举个例子：假设一共有4个cpu，此时一个java进程的%cpu列为265，则表示当前java进程占用了2.6个cpu核数。
...
* 常用的nginx服务器中如何配置keepalive功能？
  * 在nginx中配置：`keepalive_timeout 60;` 此配置表示连接的存货时间。如果空闲了60s，nginx则会主动断开。如果未配置，则默认关闭keepalive。
...
* IAAS：去云厂商购买服务器，自己安装数据库、tomcat、redis等软件。此时云厂商就是IAAS。例如云服务器

* PAAS：自己不想搭建tomcat和redis，直接在云厂商购买，由云厂商保障redis的稳定性。此时云厂商就是paas。例如阿里云的redis和ack平台

* saas：去云厂商购买**电商平台**软件，注册租户后，就可以直接使用功能了。此时就是saas。例如：钉钉以及开放平台的各种商家软件
...
* 容灾概念：
  ...
  * 机器级：一台机器挂了，其他机器顶上
  * 机房级：一个机房挂了，另外一个机房能顶上
  * 地域级：一个地方发生了地震，另外一个地方的机房能顶上
  * 行星级：地球没了，希望在月球的机房能顶上
  * 结论：一个机器一定位于某个地方的某个机房。对应阿里云就是：一个机器一定是在某个**地域**的某个**可用区**。==> `地域 对应 机房。可用区 对应 机房`

* 内网互通相关总结：

  * 同一个可用区（机房）的服务器是内网互通的，每个服务器都有一个内网ip。即同一可用区的机器是互访互通的。
  * 同一个地域的不同可用区，内网网络默认也是互通的（阿里云默认在两个可用区中用交换机连接了）
  * 不同地域之间，内网是不互通的。除非搞了一个专线（vpc自定义组网）网络，互通了两个地域的可用区
...
* 举例：`172.20.1.1/16` 是一个特定的主机地址(即172.20.1.1)，但它使用了 `/16` 作为子网掩码，表示它同样属于 `172.20.0.0/16` 网段内的一个地址。
...
### 10.8 本地装了docker有很多网卡，如何快速的找到本机的ip地址

* 执行命令：ifconfig | grep -5 en0  输出en0 前后5行的数据（因为en0就是本机的真实网卡，可以看到ip地址）
...
### 10.10 了解下URL编码，对签名、验签业务有好处

* 为什么要了解这个？因为在系统间交互时，有些系统为了验证身份，可能会做签名和验签操作。而签名就是把自己请求的参数说一定的算法运算得出一个签名后的字符串。验签就是对方请求过来后，自己也会对参数做同样的算法计算出一个签名串，再跟对方传递过来的签名串做对比，达到验证身份的目的。
* 在这个过程中会涉及到数据的网络传输。我们可以在http请求的url链接传递参数，也可以在body中传递参数，还可以用表单提交的方式提交参数。这里就会有一个点：涉及到url的编解码操作。如果我们把参数放在url上的话，则会经过URL的标准编码操作。如果我们把参数放在表单提交上，则会经过URL的表单编码操作。
* 举个例子，以**Hello+World**字符串为例，
...
| 流控类别   | 定义                                                         | 备注                                                         |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 并发度流控 | 比如你有一个小店，但是店里空间有限，只能容纳10个人。如果同时来了20个人，店里就会变得非常拥挤，顾客体验不好。此时可以实行一个规则：店里最多同时只能有10个顾客，如果有更多人要进来，就得在外面排队。在程序中则表示系统中同时能处理多少请求。比如每个请求处理需要100ms，那每秒的并发度就是10 | 在java中，就可以通过限制线程数量来实现并发度限流。比如一个固定大小的线程池，这样就可以限制同时运行的线程数，进而控制并发度。 |
| qps流控    | 假如你有一个小店，你的店员每秒钟能服务一个顾客，那么QPS就是1，如果每秒钟来了2个顾客，店员就忙不过来了，顾客的等待时间就会变长。此时可以实行一个规则：每秒只让一个顾客进店。 | 在JAVA中要实现qps流控，可以使用guava库的RateLimiter实现qps流控 |
|            |                                                              |                                                              |
...
* 通常：老板安排我们做一件事情的时候，或者我们主动请缨要做某个时间的时候。这件事可大可小，不管这个事情有多大，都可以按照5w2h的方法论去套。

* why：我们要做一件事情的时候，通常要说清楚：为什么要做这个事情，这个事情做和不做的带来的影响是什么。做完后，能达到什么样的效果。

* what：需要分析这个事情的现状和痛点，以此来推导我们去发现要做什么具体的事项

* when：基于这些要做的具体事项，按难、中、低的难度排序 或者 按照事情的前后依赖来定短期目标和长期目标，确定好节奏。

* who：按照短期目标和长期目标，将具体的事情分配具体的人，由他来承担这个模块的1号位

* where：确定在哪里做，如果是同一个公司的人，这个地方通常就是各自的工位了。如果是非常重大的项目，则可以申请一个项目室，所有的人都在里面办公

* how：每个具体的人对自己的模块做技术方案设计，确定如何去做。

* how much：每个具体的人对自己的模块按照短期模块做排期，如果排期比较长，可以适当变更短期目标和长期目标的时间点（即上面的when）

* 结果佐证：做完后，看是否减少了why中『不做这个事情带来的影响』。

* 举例：

  * 老板根据生态应用频繁出现的稳定性问题，给我下发了一个命题：如何保证生态的稳定性，提升C端用户体验，让商家更好的运营？

  * 先考虑why：我们得知道老板为什么要做这个事情，一定是有什么稳定性问题发生，影响了很多用户或者造成了大量的资损等等！因此需要启动这么一个项目，来提高系统的稳定性。

  * 再考虑做什么：我们可以把近期（可以是近两年、近一年、近半年）出现的稳定性问题梳理出来。梳理出来：哪个时间点发生了什么故障，故障持续了多长时间？如何快速止血的？谁来止血的？止血花费了多长时间？带来了哪些影响？发生这个故障的原因是什么？然后再基于这些案例做抽象，分析：可以通过哪些手段避免这些故障，以及针对避免不了的故障，如何来快速恢复？

    * eg：生态应用因为资源问题，服务成功率跌为0，最后联系应用owner做扩容操作，操作了2个小时后恢复。整个故障持续了2个小时。 这里面就有几点可以注意：
      * 1、服务成功率跌为0已经很滞后了，我们是否可以在成功率在同比下跌x%的时候就触发告警
      * 2、联系应用owner到扩容操作结束，一共花费了2个小时，是应用owner对扩容操作不熟悉还是因为金额不够，在扩容过程中遇到各种卡点？

    基于上面的这个案例，我们就发现有这些点可以去避免或者提前发现稳定性问题，提前布防。然后再梳理其他的稳定性问题case，做分析，可能也会得出1、2、3条注意事项。最后再把这些注意事项做分析和抽象，把一些公共问题提炼出来，变成一个个模块。这样我们就能知道要做什么具体的事项了。

  * 然后再考虑when：我们需要把这些模块按照难、中、低的难度排序，分解成短期目标和长期目标。
    * eg：我们可以把运维监控告警作为短期目标，然后把平台接入托管操作作为长期目标。（核心就是：容易实现的作为短期目标，难度大的作为长期目标。短期目标是：1-3个月可以实现的，长期目标：半年或一年才能实现）
  * 再考虑who：一般一个项目会有其他的成员一起参与，我们可以按照成员的特性来分配模块，也可以让成员自己认领模块。
  * 接着考虑where：一般非外包项目，都是在一家公司处理，如果项目很紧急的话，可以申请一个会议室作为项目作战室，大家集中在一起开发，也能提高效率
  * 接着考虑how：模块分配给成员了，就由成员来设计技术方案了。由成员来决定每个模块要如何去实现，组内再做相关的技术方案评审，让大家达成一致。
  * 最后再考虑how much：每个成员需要根据短期目标的时间点，做模块排期，如果确实需要做很久，则可以适当的改变短期目标和长期目标的时间点。

  ### 10.14 SimpleDateFormat造成的oom
  ...
  * 如上这段代码为根据传入的time和format转成Date类型的对象。
  * Date date = DateUtils.convertToDate("209410529", "YYYYMMdd"); 传入的time为209410529，不是标准的yyyyMMdd格式，但最终转化出来的date为：`Mon Mar 12 00:00:00 CST 2096` 变成了2096年了。这会带来什么影响呢？
  * 假设我们现在接收前端的开始时间和结束时间的字段，如果后端没有对时间做校验的话，我完全可以这么传递: 开始时间：20010101, 结束时间：209410529。 如果后端会根据这个时间段来构建对应的vo对象的话，那就会构建大量的数据出来，进而触发内存被大量占用。如果量太多的话，完全有可能触发oom。

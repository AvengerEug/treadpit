...
| 流控类别   | 定义                                                         | 备注                                                         |
| ---------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| 并发度流控 | 比如你有一个小店，但是店里空间有限，只能容纳10个人。如果同时来了20个人，店里就会变得非常拥挤，顾客体验不好。此时可以实行一个规则：店里最多同时只能有10个顾客，如果有更多人要进来，就得在外面排队 | 在java中，就可以通过限制线程数量来实现并发度限流。比如一个固定大小的线程池，这样就可以限制同时运行的线程数，进而控制并发度。 |
